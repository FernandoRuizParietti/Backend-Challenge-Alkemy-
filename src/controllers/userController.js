...
const tokenGenerator = async (req, res) => {
    ...
    switch (tokenCase){
        case 'resetPassword':
            DB_HOST === "localhost" ? url = `http://localhost:3001/users/resetPassword/${user.token}` : url = `http://localhost:3001/users/resetPassword/${user.token}`
            await sendMail({template:'resetPassword',email: user.email, name: user.name, url: url})
        break
        case 'validateUser':
            DB_HOST === "localhost" ? url = `http://localhost:3001/users/activate/${user.token}` : url = `http://localhost:3001/users/activate/${user.token}`
